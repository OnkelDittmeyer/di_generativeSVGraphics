		...
		function viz_a_0_1(data, id){
			...
			var path = 'M'+
						...
						(1+fa* parseInt(social[1][0])) +' '+
						(1+fa* parseInt(social[1][1])) +' l'+
						(1+fa* parseInt(social[2][0])) +' '+
						(1+fa* parseInt(social[2][1])) +' l'+						
						(1+fa* parseInt(social[3][0])) +' '+
						(1+fa* parseInt(social[3][1])) +' l'+						
						(1+fa* parseInt(social[4][0])) +' '+
						(1+fa* parseInt(social[4][1]))
						// +' Z';					
			...
			for(n=0; n<lifestyle.length; n++){
				for(i=0; i<lifestyle[n].length; i++){
						...
						if(o<5){
							...
							skewI = (10+knowledge[i][0]*10)*0.5;

							knowledgeLines.push(s.line(xPos,yPos,(10+knowledge[i][0]*10), yPos+skewI).attr({stroke:c1, strokeWidth: 1, strokeDasharray: '5 5'}));
							yPos = yPos+5;
							o++;
						}
				}			
...
//methods
			this.updateViz = function(data) {
				...
				//console.log('update 0_1');

//star update
				path = 'M'+
						(vizWidth/2 - parseInt(social[0][0])*fa) +' '+
						(vizHeight/2 - parseInt(social[0][1]*fa)) +' l'+
						(0+fa* parseInt(social[1][0])) +' '+
						(0+fa* parseInt(social[1][1])) +' l'+
						(0+fa* parseInt(social[2][0])) +' '+
						(0+fa* parseInt(social[2][1])) +' l'+						
						(0+fa* parseInt(social[3][0])) +' '+
						(0+fa* parseInt(social[3][1])) +' l'+						
						(0+fa* parseInt(social[4][0])) +' '+
						(0+fa* parseInt(social[4][1]))
