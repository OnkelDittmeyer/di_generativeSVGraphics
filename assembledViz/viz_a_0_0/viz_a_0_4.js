...
//social
			...
			for(i=0; i<360/angle; i++){
				...
				var newPath = s.path(path).attr({fill:c2, fillOpacity: 0.3, stroke: c1, strokeWeight: 1, strokeDasharray: '3 5'});
...
//background lines
			...
			for(i=0; i*(height+1)<vizHeight; i++){
						s.rect(xPos, yPos, vizWidth, height).attr({fill:c2, opacity: 0.2});
						yPos = yPos+height+1;
			}			

//			for(i=0; i*(height+5)<vizWidth; i++){
//						s.rect(xPos, yPos, height, vizHeight ).attr({fill:c2, opacity: 0.2});
//						xPos = xPos+height+5;
//			}
...
//body
		...
		for(i=0; i<body[0].length;i++){
			posY = posY - body[2][0]*0.2;
			//bodyLines.push(s.line(0, posY, vizWidth, posY).attr({stroke:c2, strokeOpacity: 0.7, strokeWidth: 1}));
			bodyPills.push(s.line(body[0][i]*scale, posY, (body[0][i]*scale) + (body[0][i]/body[1][0]*scale), posY).attr({stroke: 'white', strokeOpacity: 1, strokeWidth: 5, strokeLinecap: 'round'}));	
...
//lifestyle + knowledge
			...
			for(n=0; n<lifestyle.length; n++){
				for(i=0; i<lifestyle[n].length; i++){
						...
						lifestylePolys.push(s.polygon(xPos,yPos, xPos, yPos+height, xPos+lifestyle[n][i], yPos+height+skewI, lifestyle[n][i], yPos+skewI).attr({fill:'white', opacity: 1}));
						yPos = yPos+height+5;			
...
//boxes,//location
		...
		for(i=0; i<location.length;i++){
				yPos = yPos + parseInt(location[i][2])*(evaluateMachine(location[i][1])*0.05);
				locationRects.push(s.rect(xPos - (parseInt(location[i][3]*0.5)*0.5), yPos, parseInt(location[i][3])*0.5, parseInt(location[i][3])*0.5).attr({fillOpacity: 0.0, stroke:c1, strokeOpacity:0.8, strokeWidth:1, transform: 'R45'}));
			...
			this.updateViz = function(data) {
				...
				path = 'M'+
						...
						(9+fa* parseInt(social[1][1])) +' l'+
						(8+fa* parseInt(social[2][0])) +' '+
						(2+fa* parseInt(social[2][1])) +' l'+						
						(5+fa* parseInt(social[3][0])) +' '+
						(0+fa* parseInt(social[3][1])) +' l'+						
						(0+fa* parseInt(social[4][0])) +' '+
						(9+fa* parseInt(social[4][1]))
...
//boxes update
			...
			for(i=0; i<locationRects.length;i++){
				...
				locationRects[i].animate({x: xPos-parseInt(location[i][3])*0.5/2, y:yPos, width: (parseInt(location[i][3])*0.5), height: (parseInt(location[i][3])*0.5)},3000, mina.easeinout);		
