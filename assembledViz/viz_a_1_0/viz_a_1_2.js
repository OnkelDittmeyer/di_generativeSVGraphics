...
// rectangles green
			...
			for(i=0; i<body[0].length; i++){
				...
				bodyRects.push(s.rect(xPos, (body[0][i]*bodyScale)/(body[2][0]*0.1), 8, body[0][i]*bodyScale).attr({ strokeWeight: 3, fill: c1, fillOpacity: 0.8, transform: 'R90'}))
...
//little balls update
			...
			for(i=0; i<knowledgeCircles.length; i++){
				xPos = (xPos + parseInt(knowledge[(i+1)%knowledge.length][0]))*1.4;
