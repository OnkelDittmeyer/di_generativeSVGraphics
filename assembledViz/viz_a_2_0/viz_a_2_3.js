...
//social
			...
			for(i=0; i<social.length; i++){
				...
				socialPaths.push(s.path(path).attr({fillOpacity: 0.0, fill:c2, stroke: c2, strokeWidth: 200, strokeOpacity:0.1}))	
			};
...
//location			
			for(i = 0; i<lctn.length; i++){			
				...
				allPaths.push(s.path(path).attr({fillOpacity: 0.1, fill:c1, stroke: 'white', strokeWidth: 2, strokeOpacity: 0.5}))
				posX = posX +0;		
			};			
...
//body
		...
		for(i=0; i<body[0].length; i++){
			allBodyRects.push(s.rect(posX + parseInt(body[0][i])/(parseInt(body[2][0])*6),posY,parseInt(body[0][i])/(parseInt(body[1][0]*2)), 10).attr({fill:'white', opacity:0.3}));		
			posY = posY + parseInt(body[0][i])/(parseInt(body[2][0])*6);
		};
...
// white rects		
			...
			for(i=0; i<allBodyRects.length; i++){
				...
				allBodyRects[i].animate({x:posX + parseInt(body[0][i])/(parseInt(body[2][0])*6), y: posY, width: parseInt(body[0][i])/(parseInt(body[1][0])*2)}, 3000);		
				posY = posY + 10;
				//allBodyRects.animate(s.rect(posX + parseInt(body[0][i])/(parseInt(body[2][0])*6),posY,parseInt(body[0][i])/(parseInt(body[1][0]*2)), 10).attr({fill:'white', opacity:0.3}));		
			};
